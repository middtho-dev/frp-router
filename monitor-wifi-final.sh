...
echo "[1] Установить Wi-Fi Telegram Monitor"
echo "[2] Удалить Wi-Fi Telegram Monitor"
echo -n "Введите номер: "
read choice

if [ "$choice" -eq 2 ]; then
    echo "[*] Удаление Wi-Fi Telegram Monitor..."
    ...
    rm -f /etc/init.d/wifi-monitor
    rm -f /etc/wifi-monitor/wifi-monitor.sh

    # Удаление конфигурации
    rm -f /etc/wifi-monitor/devices.conf  # Если вы использовали конфиг с MAC-адресами
    rm -f /tmp/wifi_monitor_clients
    rm -f /tmp/wifi_monitor_current

    # Удаление cron-задачи
    sed -i '/wifi-monitor-check/d' /etc/crontabs/root
