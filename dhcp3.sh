...
get_current_clients() {
    awk '{print $2, $3}' /tmp/dhcp.leases | sort
}

# Получаем информацию о DHCP-клиенте по MAC-адресу
get_ip_hostname() {
    MAC="$1"
    grep -i "$MAC" /tmp/dhcp.leases | awk '{printf "%s|%s|%s", $3, $4, $2}'  # Возвращаем IP, имя устройства и время аренды
}
...
while true; do
    ...
    for mac in $(comm -13 "$CLIENT_LIST" "$CURRENT_LIST"); do
        ...
        timestamp=$(echo "$info" | cut -d'|' -f3)  # Время последнего обновления аренды
        ...
        # Расчёт разницы во времени (в секундах)
        lease_time=$(($current_time - $timestamp))

        # Конвертация времени аренды в формат часы:минуты:секунды
        formatted_time=$(convert_time "$lease_time")

        send_msg "✅ *Подключился к $DEVICE_NAME*
📱 Имя: *$name*
🌐 IP: \`$ip\`
🔗 MAC: \`$mac\`
⏰ Время аренды: $formatted_time"
    done

    # Обработка отключений
    for mac in $(comm -23 "$CLIENT_LIST" "$CURRENT_LIST"); do
        # Получаем информацию о отключившемся устройстве
        info=$(get_ip_hostname "$mac")
        ip=$(echo "$info" | cut -d'|' -f1)
        name=$(echo "$info" | cut -d'|' -f2)
...
start() {
    ...
    /etc/dhcp-monitor/dhcp-monitor.sh &  # Запуск основного скрипта
}

stop() {
    echo "[-] Остановка dhcp-monitor"
    killall dhcp-monitor.sh 2>/dev/null  # Остановка скрипта
}
EOF
