...
# ===============================
remove_all() {
    echo -e "${RED}Удаление FRPC и всех компонентов...${NC}"
    ...
    send_telegram "🗑️ FRPC удалён с *$(uname -n)*"
...
echo -e "${GREEN}Устройство:${NC} $DEVICE_NAME"
...
# ===============================
# 📡 ЗАПРОС ПОРТОВ У БОТА
# ===============================
echo -e "${GREEN}Запрос портов у сервера (bot → /allocate)...${NC}"
...
echo -e "${YELLOW}DEBUG: PARSED:${NC}"
echo "PORTS     = $PORTS"
echo "LUCi_PORT = $LUCi_PORT"
echo "SSH_PORT  = $SSH_PORT"

# ===============================
# ❌ ЖЁСТКАЯ ПРОВЕРКА
# ===============================
if [[ -z "$PORTS" || "$PORTS" == "null" || ! "$PORTS" =~ ^[0-9]+:[0-9]+$ ]]; then
    echo -e "${RED}❌ БОТ НЕ ВЫДАЛ ПОРТ${NC}"
    echo "Проверь:"
    echo "• бот запущен"
    echo "• ADMIN_ID совпадает"
    echo "• команда /allocate есть в коде"
    exit 1
fi

echo -e "${GREEN}✅ Порты получены успешно${NC}"
